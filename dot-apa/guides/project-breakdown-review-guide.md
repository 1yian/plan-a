# APA 0.1.0 - Project Breakdown Review Guide
...
- Ensure the `implementation-plan.md` is in its final, clean state.
- Confirm all task headers, agent assignments, and dependency tags are formatted correctly.

**Bootstrap Prompt Generation:**
- Pass control back to the .claude/commands/apa.init.md logic.
- **Context Recovery:** When generating the Bootstrap Prompt, you must use the **EXACT TEMPLATE** from .claude/commands/apa.init.md. If the template is degraded or missing from your context window, **READ .claude/commands/apa.init.md** to retrieve it before generating the final artifact.
