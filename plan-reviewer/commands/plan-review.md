...
4. **Create a working copy in cwd**: Use `cp` command to copy the original plan to `./{original_name}.review.md` in the current working directory (e.g., `cp .claude/plans/my-feature.md ./my-feature.review.md`). Do NOT use Read/Write - use Bash with cp.
