...
4. **Create a working copy in cwd**: Do NOT read the plan first. Use `cp` or equivalent command to copy the original plan to `./{original_name}.review.md` in the current working directory (e.g., `cp .claude/plans/my-feature.md ./my-feature.review.md`).
...
All 6 use the same agent (`plan-reviewer:reviewer`) with different persona prompts. This agent has `model: inherit` and `thinking_budget: extended` for deep analysis.
